...
await writeFile(path.join(releaseDirectory, 'release-manifest.json'), `${JSON.stringify({ schemaVersion: 1, product: 'Tempest Streaming Studio', version: expectedVersion, platform: 'win32', arch: 'x64', generatedAt: new Date().toISOString(), signed: false, artifacts }, null, 2)}\n`);
console.log(`TEMPEST_RELEASE_VERIFIED ${expectedVersion} ${artifacts.map((entry) => `${entry.name}:${entry.size}`).join(' ')}`);
