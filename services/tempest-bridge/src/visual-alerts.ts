...
<head>
  ...
  <style id="baseStyle">
    :root{color-scheme:dark;--accent:#54f2eb;--text:#f5fbff;--secondary:#a9bdc7;--background:rgba(5,12,19,.94);--font:Inter;--font-size:42px;--font-weight:800;--letter-spacing:0px;--text-shadow:0.35;--card-width:900px;--border-width:1px;--border-radius:24px;--padding:22px;--card-shadow:.55;--media-width:320px;--media-height:210px;--media-radius:16px;--media-fit:contain;--text-x:0px;--text-y:0px;--text-anchor-x:0px;--text-anchor-y:0px;--eyebrow-position-x:50%;--eyebrow-position-y:52%;--headline-position-x:50%;--headline-position-y:63%;--detail-position-x:50%;--detail-position-y:74%;--message-position-x:50%;--message-position-y:84%}
...
export class TempestVisualAlertOverlay {
  ...
  showTwitch(alert: TempestTwitchVisualAlertDefinition, event: TempestNormalizedTwitchEvent, runId = event.id): TempestVisualAlertEvent {
    ...
    return this.activate({
      ...
      ...(alert.visualUri ? { mediaUrl: `/visual-alerts/media/${encodeURIComponent(alert.id)}`, mediaKind: mediaKind(alert.visualUri) } : {}),
      ...(alert.audioUri ? { audioUrl: `/visual-alerts/audio/${encodeURIComponent(alert.id)}`, audioDurationMs: alert.durationMs, volume: alert.volume } : {}),
      startedAt: new Date().toISOString()
    });
  }
