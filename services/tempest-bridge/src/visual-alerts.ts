...
<head>
  ...
  <style id="baseStyle">
    ...
    *{box-sizing:border-box}html,body{width:100%;height:100%;margin:0;overflow:hidden;background:transparent;font-family:Inter,Segoe UI,sans-serif}.stage{display:flex;width:100%;height:100%;padding:4vh 4vw;pointer-events:none}.placement{flex:none;transform:translate(var(--stage-x,0px),var(--stage-y,0px)) scale(var(--alert-scale,1));transform-origin:center}.stage.custom{position:relative;display:block;padding:0}.stage.custom .placement{position:absolute;left:calc(var(--custom-x,50%) + var(--stage-x,0px));top:calc(var(--custom-y,82%) + var(--stage-y,0px));transform:translate(-50%,-50%) scale(var(--alert-scale,1))}body[data-position^="top-"] .stage{align-items:flex-start}body[data-position^="center"] .stage{align-items:center}body[data-position^="bottom-"] .stage{align-items:flex-end}body[data-position$="left"] .stage{justify-content:flex-start}body[data-position$="-center"] .stage,body[data-position="center"] .stage{justify-content:center}body[data-position$="right"] .stage{justify-content:flex-end}
    ...
    body[data-orientation="vertical"] .stage{padding:5vh 7vw}body[data-orientation="vertical"] .alert[data-layout="media-left"],body[data-orientation="vertical"] .alert[data-layout="media-right"]{grid-template-columns:1fr;width:min(var(--card-width),86vw);max-height:58vh}body[data-orientation="vertical"] .alert[data-layout="media-left"] .media,body[data-orientation="vertical"] .alert[data-layout="media-right"] .media{order:1;width:min(100%,var(--media-width));height:min(var(--media-height),27vh);justify-self:center}body[data-orientation="vertical"] .alert[data-layout="media-left"] .copy,body[data-orientation="vertical"] .alert[data-layout="media-right"] .copy{order:2;text-align:center}body[data-orientation="vertical"] .alert[data-layout="media-overlay"]{width:min(var(--media-width),86vw);height:min(var(--media-height),58vh)}
...
<body data-position="bottom-center">
  ...
  <script>
    (()=>{
      const root=document.documentElement,body=document.body,stage=document.getElementById('stage'),placement=document.getElementById('placement'),card=document.getElementById('alert'),media=document.getElementById('media'),copy=document.getElementById('copy'),eyebrow=document.getElementById('eyebrow'),name=document.getElementById('name'),detail=document.getElementById('detail'),message=document.getElementById('message'),customHtml=document.getElementById('customHtml'),audio=document.getElementById('alertAudio'),customStyle=document.getElementById('customStyle'),orientation=new URLSearchParams(location.search).get('orientation')==='vertical'?'vertical':'horizontal';body.dataset.orientation=orientation;
      ...
      let timers=[],revision=0,runtimeStopTimer,audioContext,audioSource,audioGain,audioFetchController;
      ...
      function stopAudio(){if(runtimeStopTimer){clearTimeout(runtimeStopTimer);runtimeStopTimer=undefined}if(audioFetchController){audioFetchController.abort();audioFetchController=undefined}if(audioSource){try{audioSource.stop()}catch{}try{audioSource.disconnect()}catch{}audioSource=undefined}if(audioGain){try{audioGain.disconnect()}catch{}audioGain=undefined}audio.pause();audio.removeAttribute('src');audio.load();if('speechSynthesis'in window)window.speechSynthesis.cancel()}
      ...
      async function playWebAudio(url,volume,current){const AudioContextClass=window.AudioContext||window.webkitAudioContext;if(!AudioContextClass)throw new Error('Web Audio is unavailable');if(!audioContext||audioContext.state==='closed')audioContext=new AudioContextClass({latencyHint:'interactive'});if(audioContext.state==='suspended')await audioContext.resume();const controller=new AbortController();audioFetchController=controller;const response=await fetch(url,{cache:'no-store',signal:controller.signal});if(!response.ok)throw new Error('HTTP '+response.status);const encoded=await response.arrayBuffer();if(audioFetchController===controller)audioFetchController=undefined;if(revision!==current)return false;const decoded=await audioContext.decodeAudioData(encoded.slice(0));if(revision!==current)return false;const source=audioContext.createBufferSource(),gain=audioContext.createGain();gain.gain.value=volume;source.buffer=decoded;source.connect(gain);gain.connect(audioContext.destination);audioSource=source;audioGain=gain;source.onended=()=>{if(audioSource===source)audioSource=undefined;if(audioGain===gain)audioGain=undefined;try{source.disconnect()}catch{}try{gain.disconnect()}catch{}};source.start(0);return true}
      async function playElementAudio(url,volume){audio.src=url;audio.volume=volume;audio.load();let timeout;try{await Promise.race([audio.play(),new Promise((_,reject)=>{timeout=setTimeout(()=>reject(new Error('media element timed out while loading')),4000)})])}finally{if(timeout)clearTimeout(timeout)}}
      async function playAudio(data,current){if(revision!==current||!data.audioUrl)return;const url=data.audioUrl+(data.audioUrl.includes('?')?'&':'?')+'run='+encodeURIComponent(data.runId||Date.now()),volume=Math.max(0,Math.min(1,Number(data.volume)||0));let webAudioError;try{if(await playWebAudio(url,volume,current))return}catch(error){webAudioError=error;if(revision!==current||error&&error.name==='AbortError')return}if(revision!==current)return;try{await playElementAudio(url,volume)}catch(error){if(revision!==current)return;const primary=webAudioError instanceof Error?webAudioError.message:String(webAudioError||'unknown error'),fallback=error instanceof Error?error.message:String(error);console.error('Tempest alert audio playback failed: Web Audio: '+primary+'; media fallback: '+fallback)}}
      ...
      function playAudio(data,current){if(orientation==='vertical'||revision!==current||!data.audioUrl)return;audio.src=data.audioUrl+(data.audioUrl.includes('?')?'&':'?')+'run='+encodeURIComponent(data.runId||Date.now());audio.volume=Math.max(0,Math.min(1,Number(data.volume)||0));audio.play().catch(()=>{});later(()=>{if(revision===current)stopAudio()},Math.max(1000,Number(data.audioDurationMs)||60000))}
      ...
      function show(data){
        ...
        const verticalPosition=orientation==='vertical'&&String(design.position).startsWith('bottom')?String(design.position).replace('bottom','center'):design.position;const customX=orientation==='vertical'?Math.max(12,Math.min(88,Number(design.customPositionX)||50)):design.customPositionX;const customY=orientation==='vertical'?Math.max(10,Math.min(62,Number(design.customPositionY)||50)):design.customPositionY;body.dataset.position=verticalPosition;stage.classList.toggle('custom',design.position==='custom');placement.style.setProperty('--stage-x',design.positionOffsetX+'px');placement.style.setProperty('--stage-y',design.positionOffsetY+'px');placement.style.setProperty('--custom-x',customX+'%');placement.style.setProperty('--custom-y',customY+'%');placement.style.setProperty('--alert-scale',design.scale);card.dataset.preset=design.preset;card.dataset.layout=design.layout;card.dataset.enter=design.entranceAnimation;card.dataset.exit=design.exitAnimation;copy.dataset.textAnimation=design.textAnimation;customStyle.textContent=design.customCss||'';
