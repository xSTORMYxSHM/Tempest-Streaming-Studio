...
const stage=document.getElementById('stage');let settings={};let participants=[];const cards=new Map();function initials(v){return String(v||'?').trim().split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase()}function media(p){let kind='idle';if(p.deaf&&p.deafenAssigned)kind='deafen';else if(p.mute&&p.muteAssigned)kind='mute';else if(p.speaking&&p.speakingAssigned)kind='speaking';if(kind!=='idle'||p.idleAssigned)return './discord-voice/media/'+encodeURIComponent(p.id)+'/'+kind+'?v='+encodeURIComponent(p.profileUpdatedAt||'');return p.avatarUrl||''}function createCard(p){const card=document.createElement('article');card.className='person';card.innerHTML='<div class="portrait"><img class="hidden" alt=""><span class="initials"></span><span class="status"></span></div><strong class="name"></strong>';const image=card.querySelector('img');const fallback=card.querySelector('.initials');image.addEventListener('error',()=>{image.classList.add('hidden');fallback.classList.remove('hidden')});cards.set(p.id,card);return card}function updateImage(card,src){const image=card.querySelector('img');const fallback=card.querySelector('.initials');if(image.dataset.source!==src){image.dataset.source=src;if(src){image.classList.remove('hidden');fallback.classList.add('hidden');image.src=src}else{image.removeAttribute('src');image.classList.add('hidden');fallback.classList.remove('hidden')}}}function updateCard(card,p,index,total){card.classList.toggle('speaking',p.speaking===true);card.style.setProperty('--person-accent',p.accent||settings.speakingAccent);card.style.setProperty('--person-x',(p.positionX??((index+1)/(total+1)*100))+'%');card.style.setProperty('--person-y',(p.positionY??82)+'%');card.querySelector('.initials').textContent=initials(p.displayName);const name=card.querySelector('.name');name.textContent=p.displayName;name.classList.toggle('hidden',settings.showNames===false);const status=card.querySelector('.status');status.replaceChildren();if(settings.showStatusIcons&&p.mute){const badge=document.createElement('i');badge.title='Muted';badge.textContent='MUTE';status.appendChild(badge)}if(settings.showStatusIcons&&p.deaf){const badge=document.createElement('i');badge.title='Deafened';badge.textContent='DEAF';status.appendChild(badge)}updateImage(card,media(p))}function render(){const visible=(settings.enabled===false?[]:participants).filter(p=>p.visible!==false&&!(settings.hideSelf&&p.self)&&!(settings.hideBots&&p.bot)).sort((a,b)=>(a.order-b.order)||a.displayName.localeCompare(b.displayName));const visibleIds=new Set(visible.map(p=>p.id));for(const [id,card] of cards)if(!visibleIds.has(id)){card.remove();cards.delete(id)}visible.forEach((p,index)=>{const card=cards.get(p.id)||createCard(p);updateCard(card,p,index,visible.length);const current=stage.children[index];if(current!==card)stage.insertBefore(card,current||null)})}function apply(next){settings={...settings,...next};const root=document.documentElement.style;root.setProperty('--size',(settings.avatarSize||160)+'px');root.setProperty('--gap',(settings.gap||24)+'px');root.setProperty('--inactive',String(settings.inactiveOpacity??.72));root.setProperty('--scale',String(settings.speakingScale??1.08));root.setProperty('--accent',settings.speakingAccent||'#54f2eb');root.setProperty('--speed',(settings.transitionMs||140)+'ms');stage.className=settings.layout||'horizontal';render()}const events=new EventSource('./discord-voice/events');events.addEventListener('init',e=>{const d=JSON.parse(e.data);participants=d.participants||[];apply(d.settings||{})});events.addEventListener('settings',e=>apply(JSON.parse(e.data)));events.addEventListener('state',e=>{participants=JSON.parse(e.data).participants||[];render()});events.onerror=()=>{};
})();</script></body></html>`;
