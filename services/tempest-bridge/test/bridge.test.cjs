...
test('persists applications and assets behind authenticated routes', async (context) => {
  ...
  assert.equal(health.productVersion, '1.2.6');
...
test('owns a free Sound Alert catalog, configuration, playback, and emergency stop', async (context) => {
  ...
  const rememberedState = await discordState.json();
  assert.equal(rememberedState.guests[0].inChannel, true);
  assert.equal(rememberedState.profiles[0].discordDisplayName, 'Guest');
  assert.equal(rememberedState.profiles[0].lastChannelName, 'Creator Lounge');
  ...
  assert.equal(discordStatus.guests.find((guest) => guest.id === offlineUserId).displayName, 'Discord user …5678');
  assert.equal(discordStatus.guests.find((guest) => guest.id === offlineUserId).inChannel, false);
  assert.equal((await fetch(`${runtime.baseUrl}/v1/discord-voice/profiles/${offlineUserId}`, { method: 'DELETE', headers })).status, 200);
