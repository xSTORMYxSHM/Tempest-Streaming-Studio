...
test('renderer includes orchestration and management sections with a restrictive content security policy', async () => {
  ...
  assert.match(renderer, /renderInterfaceMode/);
  assert.match(styles, /body:not\(\.advanced-mode\) \[data-advanced\]/);
  ...
  assert.match(html, /id="twitchDesignSnap"/);
  assert.match(html, /id="alignAlertHorizontal"/);
  assert.match(html, /id="alignAlertVertical"/);
  assert.match(html, /id="previewAlertDesignOnCanvas"/);
  assert.match(html, /id="alertDesignSceneScope"/);
  assert.match(html, /Use current Broadcast scene/);
  assert.match(html, /Remove scene placement/);
  assert.match(html, /Show on Canvas/);
  assert.match(renderer, /snapAlertCoordinate/);
  assert.match(renderer, /\/v1\/visual-alerts\/position-preview/);
  assert.match(renderer, /alertDesignScenePlacements/);
  ...
  assert.match(html, /TWITCH SIGN-IN/);
  ...
  assert.match(html, /STREAM TOGETHER COLLABORATION CENTER/);
  ...
  assert.match(renderer, /\/v1\/integrations\/kick\/oauth\/start/);
  assert.match(renderer, /linkHostedKick/);
  assert.match(main, /kick-credentials\.bin/);
  assert.match(preload, /studio:link-hosted-kick/);
  ...
  assert.match(renderer, /mute duplicate alert audio/);
  ...
  assert.match(html, /class="simulcast-manual-check"/);
  assert.match(html, /id="simulcastSessionClock"/);
  assert.match(html, /id="simulcastTelemetryFreshness"/);
  assert.match(html, /id="simulcastIncidentTimeline"/);
  assert.match(html, /never renders video, changes encoders/i);
  ...
  assert.match(renderer, /\/v1\/broadcast\/simulcast\/stop/);
  assert.match(renderer, /operatorChecklistAccepted/);
  assert.match(renderer, /sessionStorage\.setItem\(simulcastChecklistStorageKey/);
  ...
  assert.match(html, /Streamer Mode/);
  assert.match(html, /Show Advanced Mode/);
  ...
  assert.match(renderer, /Pause Viewer Effects/);
  ...
  assert.match(main, /protocol\.handle\('tempest'/);
  assert.match(main, /assertTrustedDesktopSender/);
